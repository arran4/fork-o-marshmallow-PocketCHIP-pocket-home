...
static int pocket_i2c_smbus_access(int file, char readWrite, uint8_t command,
...
static int pocket_i2c_smbus_write_byte_data(int file, uint8_t command,
                                            uint8_t value) {
    union i2c_smbus_data data;
    data.byte = value;
    return pocket_i2c_smbus_access(file, I2C_SMBUS_WRITE, command,
                                   I2C_SMBUS_BYTE_DATA, &data);
}

static int pocket_i2c_smbus_read_byte_data(int file, uint8_t command) {
    union i2c_smbus_data data;
    if (pocket_i2c_smbus_access(file, I2C_SMBUS_READ, command,
...
uint8_t i2c_write_byte(int file, uint8_t reg, uint8_t byte) {
    int res = pocket_i2c_smbus_write_byte_data(file, reg, byte);
...
uint8_t i2c_read_byte(int file, uint8_t reg) {
    int data = pocket_i2c_smbus_read_byte_data(file, reg);
    if (data < 0)
        printf("Error: %s\n", strerror( errno ) );
